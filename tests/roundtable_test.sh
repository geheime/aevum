#!/usr/bin/env bash
# roundtable_test.sh — regression suite for bin/roundtable.sh.
# Every case below is a failure mode a red team reproduced against the parser.
# Run:  bash tests/roundtable_test.sh   (exit 0 = all green)
set -uo pipefail
DIR="$(cd "$(dirname "$0")" && pwd)"
RT="$DIR/../bin/roundtable.sh"
...
EOF
o=$("$RT" live "$TMP/t1"); check "happy: live front shows" "dev @ deploy" "$o"
...
EOF
o=$("$RT" live "$TMP/t2"); check "claim+release: clean" "0 live fronts" "$o"
...
EOF
o=$("$RT" live "$TMP/t5"); check "sep-in-front: full front preserved" "web pre-live · lista + FAQ" "$o"
