...
# 1 · CRIT1 — reclaim lives in adapters/claude-code/, mesa.sh in bin/ (NOT siblings).
#     It must still resolve the parser and SEE the live front — never silently "MESA limpia".
mkdir -p "$TMP/proj_a"
cat > "$TMP/mesa1" <<'EOF'
# MESA
2026-09-14 10:00 · dev @ deploy prod · CLAIM · sesión dead01 · start
EOF
o=$(RECLAIM_PROJECTS="$TMP/proj_a" bash "$RECLAIM" claims "$TMP/mesa1" 2>&1)
refute "CRIT1: not a silent false-clean" "MESA clean" "$o"
...
EOF
o2=$(RECLAIM_PROJECTS="$TMP/proj_b" bash "$RECLAIM" claims "$TMP/mesa2" 2>&1)
