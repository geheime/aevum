...
- **"Two agents overwrote each other."** You run agents in parallel and their
  work collides. → the **round table** shows who is on what right now, and a
...
- **Round table** — the append-only board where parallel instances coordinate
  *right now* (looks sideways): the shared board where everyone sitting can see
  who is on what.
...
- **[bin/roundtable.sh](bin/roundtable.sh)** — the round-table parser (portable, `bash` + `awk`).
...
git, CLAIMs its front on the round table, and works. At close it writes the baton back
